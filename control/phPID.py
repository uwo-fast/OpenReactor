...
class feedback:
    ...
    def getData(self):
        ...
        data, self.params["er"] = PID(
            float(self.params["input"]),
            float(self.params["lastInput"]),
    ...
    def getParams(self):
        params = (
            ...
            .order_by(ControlReading.time)[-1]
            .params
        )
        self.params = params
    ...
    def params2data(self):
        if (
            float(self.params["input"]) - float(self.params["setpoint"]) < 0
        ):  # if the solution is below target pH, want to add base so use output byte for base pump
