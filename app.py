...
from sensor.model.model import (
    Sensor,
    SensorReading,
    Control,
    ControlReading,
...
):  # if the server has been shut down but there's still an experiment running, automatically resume data aq.
...
def update(graphID, name):
    ...
    if request.method == "GET":
        ...
        return json.dumps(
            {"Time": time, "Values": values, "Title": title, "Parsed": toParse}
        )
...
def graphsUpdate(toDisplay, selected):
    ...
    if toDisplay == "-1":
        ...
        sen, timeStart, timeEnd, running = exp.info(
            selected
        )  # gets the information for the given experiment
        if (
            timeStart == -1
        ):  # if the experiment hasn't ended, either still running or hasn't been started. Use current time.
        ...
        if (
            ...
            if timeEnd - timeStart > 3600:
                timeStart = (
                    datetime.fromtimestamp(time.time()) - timedelta(hours=1)
                ).timestamp()
        sen, dataData, dataTime = experiment_entries(
            timeStart, timeEnd, toDisplay
        )  # returns the data measurements during time interval
        values1 = dataData
        time1 = dataTime
        first = time.time()
        if len(time1) != 0:
...
def add():
    ...
    if (
        data == 0
    ):  # if no graphs are displayed then toDisplay should be cleared to reflect this
        toDisplay.clear()
    if (
        len(toDisplay)
    ) == data:  # ensure that the number of graphs on frontend and backend are the same, then adds new empty graph to array
        toDisplay.append("-1")
    return flask.jsonify(data)
